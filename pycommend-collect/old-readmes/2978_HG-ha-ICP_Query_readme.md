...
6. 禁止售卖本项目，禁止用于违法目的，默认使用统一session，无法高并发

### 部署
#### 1. 自动打码，20240225版本（yolo8+未转化孪生神经网络，非生产环境），性能提升，支持替换模型，数据集是手动生成的，存在误差。权重文件可以二次训练，欢迎贡献更好的模型，或者提供更真实、全面的数据集来优化模型。该发布方式不适用于生产环境。
