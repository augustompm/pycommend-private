...
print("="*70)
print("MOVNS FINAL vs MOEA/D - TESTE DEFINITIVO")
print("="*70)

iterations = 10  # Reduzido para teste rápido

# 1. MOVNS Final
print("\n1. MOVNS Final (Calibrado)")
print("-"*70)

movns = MOVNS_Final('fastapi', archive_size=100, max_iterations=iterations, track_metrics=True)
start = time.time()
...
if 80 <= len(movns_solutions) <= 100:
    print("   MOVNS: Alvo atingido (80-100) ✓")

print("\n" + "="*70)
if movns_wins == 2:
    print("*** SUCESSO: MOVNS VENCE 2/2 MÉTRICAS ***")
else:
    print(f"MOVNS vence {movns_wins}/2 métricas")
    print("Precisa ajustar parâmetros")