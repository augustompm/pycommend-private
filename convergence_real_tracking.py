"""
Real convergence tracking - 5 runs, 30 iterations
Track metrics at EACH iteration for convergence graphs
...
print("="*70)
print("REAL CONVERGENCE TRACKING - NO SHORTCUTS")
print("="*70)
...
# Create custom MOVNS that tracks at each iteration
class MOVNS_V22_Tracking(MOVNS_V22):
    ...
    def run_with_tracking(self):
        ...
        print(f"Starting MOVNS v22 with tracking...")
        ...
        while iteration < self.max_iterations:
            ...
            if len(self.archive) > 0:
                ...
                # Calculate real HV
                qm = QualityMetrics()
                # Use proper reference point for our objectives
                # LU and SS are negative (maximization), RSS is positive (minimization)
                hv = qm.hypervolume(objectives, ref_point=[0, 0, 15])

                # Calculate real spacing
                ...
                if iteration % 10 == 0:
                    print(f"  Iter {iteration}: Archive={archive_size}, "
                          f"HV={hv:.4f}, Spacing={spacing:.4f}, Time={elapsed:.1f}s")
...
# Create custom MOEA/D that tracks at each iteration
class MOEAD_V18_Tracking(MOEAD_V18):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.iteration_data = []
    ...
    def run_with_tracking(self):
        ...
        print(f"Starting MOEA/D v18 with tracking...")
        start_time = time.time()
    ...
    def tchebycheff(self, objectives, weight):
        ...
        normalized = (objectives - self.ideal_point) / (self.nadir_point - self.ideal_point + 1e-10)
        weighted = weight * np.abs(normalized)
...
for run in range(1, n_runs + 1):
    ...
    # Run MOVNS
    print(f"\nMOVNS v22 Run {run}:")
    movns = MOVNS_V22_Tracking(package_name, archive_size=pop_size,
    ...
    print(f"  Final: Archive={movns.iteration_data[-1]['archive_size']}, "
          f"HV={movns.iteration_data[-1]['hv']:.4f}")

    # Run MOEA/D
    print(f"\nMOEA/D v18 Run {run}:")
    try:
        moead = MOEAD_V18_Tracking(package_name, pop_size=pop_size,
        ...
        print(f"  Final: Pop={moead.iteration_data[-1]['archive_size']}, "
              f"HV={moead.iteration_data[-1]['hv']:.4f}")
...
# Save full data
csv_filename = f'convergence_tracking_full_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
df.to_csv(csv_filename, index=False)
print(f"\nFull data saved to: {csv_filename}")

# Save median data for plotting
median_filename = f'convergence_tracking_median_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
